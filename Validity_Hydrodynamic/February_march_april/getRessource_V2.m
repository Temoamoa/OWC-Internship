function SS_data = getRessource_V2(Time,RBR,Bimep)
    Ts = 0.5;
    % Find data
    ...
    idx = find(datenum(RBR.date)>=Tin & datenum(RBR.date)<Tout);
    %dates = [RBR.date(idx(1)); RBR.date(idx(end))];
    
    % Collect data
    SS_data.welev = RBR.welev(idx);
    Tide = mean(RBR.tide(idx))-7.3; % Set reference tide to the min of measured tides
    %SS_data.date = dates;
    
    % Get SS characteristics
    [ f, Sxx ] = specham( SS_data.welev, Ts, 16 )	;  
    [Te, Tp]	= specseastate( f, Sxx ); 	
    Hs = H13T13(SS_data.welev,Ts);
    
    % Get Bimep wave direction check if less than 30mn
    if (Tout-Tin) <= 0.0208 % Equivalent to 1/2h
        idx = find(datenum(Bimep.date(:,2))>Tin-0.0025 & datenum(Bimep.date(:,2))<Tout+0.0025);
    else    
        idx = find(datenum(Bimep.date(:,2))>Tin & datenum(Bimep.date(:,2))<Tout);
    end
    if isempty(idx)
        idx = find(datenum(Bimep.date(:,2))>Tin-0.01 & datenum(Bimep.date(:,2))<Tout+0.01);
        sprintf('Timestamp for Direction extended');
    end
    
    Dir = Bimep.DirTp(idx(end));
    
    SS_data.SS = [Hs, Tp, Te, Dir, Tide];
    SS_data.Spectre = [ f, Sxx ];
end
